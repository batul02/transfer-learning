...
rf_en.fit(X_source_scaled, y_source)
...
explainer_en = LimeTabularExplainer(X_source_scaled, feature_names=[f'f{i}' for i in range(X_source_scaled.shape[1])],
                                     class_names=np.unique(y_source), verbose=False, mode='classification')

explainer_fr = LimeTabularExplainer(X_target_train, feature_names=[f'f{i}' for i in range(X_target_scaled.shape[1])],
                                     class_names=np.unique(y_target), verbose=False, mode='classification')
...
labels = ["Azn"] * lime_en_summary.shape[0] + ["DSLR"] * lime_fr_summary.shape[0]
